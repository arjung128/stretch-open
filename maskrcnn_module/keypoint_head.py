...
def build_keypoint_head(cfg, input_shape, corner_idx):
    """
    Build a keypoint head from `cfg.MODEL.ROI_KEYPOINT_HEAD.NAME`.
    """
    name = cfg.MODEL.ROI_KEYPOINT_HEAD.NAME
    return ROI_KEYPOINT_HEAD_REGISTRY.get(name)(cfg, input_shape, corner_idx)


def keypoint_rcnn_loss(pred_keypoint_logits, instances, normalizer, corner_idx):
    """
    Arguments:
    ...
    for instances_per_image in instances:
        ...
        if corner_idx == 0:
            ...
            keypoints = instances_per_image.gt_keypoints_1
        elif corner_idx == 2:
            keypoints = instances_per_image.gt_keypoints_2
        elif corner_idx == 3:
            keypoints = instances_per_image.gt_keypoints_3
        elif corner_idx == 4:
            keypoints = instances_per_image.gt_keypoints_4
        else:
            assert False, "corner_idx > 4 does not make sense"
...
class BaseKeypointRCNNHead(nn.Module):
    ...
    @configurable
    def __init__(self, *, num_keypoints, loss_weight=1.0, loss_normalizer=1.0, corner_idx=0):
        """
    ...
    @classmethod
    def from_config(cls, cfg, input_shape, corner_idx):
        ret = {
    ...
    def forward(self, x, instances: List[Instances]):
        ...
        if self.training:
            ...
            suffix = '' if self.corner_idx == 0 else f'_{self.corner_idx}'
            return {
                f"loss_keypoint{suffix}": keypoint_rcnn_loss(x, instances, normalizer=normalizer, corner_idx=self.corner_idx)
                * self.loss_weight
            }
        else:
...
class KRCNNConvDeconvUpsampleHead(BaseKeypointRCNNHead, nn.Sequential):
    ...
    @configurable
    def __init__(self, input_shape, *, num_keypoints, conv_dims, corner_idx, **kwargs):
        """
        NOTE: this interface is experimental.
    ...
    @classmethod
    def from_config(cls, cfg, input_shape, corner_idx):
        ret = super().from_config(cfg, input_shape, corner_idx)
